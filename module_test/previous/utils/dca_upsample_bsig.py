...
from module_test.previous.bing_sigmoid import *
...
class CCSABlock(nn.Module):
    def __init__(self, 
        ...
        if self.channel_att:
            ...
            self.c_attention = nn.ModuleList([CBAM(feature) for feature, head in zip(features, channel_head)])
        # if self.spatial_att:
        #     self.spatial_norm = nn.ModuleList([nn.LayerNorm(in_features,
        #                                                     eps=1e-6)
        #                                        for in_features in features])
...
class DCA(nn.Module):
    def __init__(self,
        ...
        # self.upconvs = nn.ModuleList([DySample_UP(in_channels=feature,scale=stride,style='lp')
        #                                             for feature, stride in zip(features, strides)])
