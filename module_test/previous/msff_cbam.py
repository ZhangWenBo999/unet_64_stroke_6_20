...
from module_test.previous.CBAM import *
# from CBAM import *

class msff_cbam(nn.Module):
    def __init__(self, dim, levels=4, group_kernel_sizes=[3, 5, 7, 9], qkv_bias=False, attn_drop_ratio=0.2):
        super(msff_cbam, self).__init__()
        self.dim = dim
        ...
        self.cbam = CBAM(gate_channels=dim)
    ...
    def forward(self, x):
        ...
        outputs = self.cbam(fused_features)
        # # 通道注意力
