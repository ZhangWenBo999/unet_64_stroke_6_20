...
class Palette(BaseModel):
    ...
    def val_step(self):
        ...
        path = os.path.join(self.opt['path']['results'], 'val')
        ...
        with torch.no_grad():
            for val_data in tqdm.tqdm(self.val_loader):
                ...
                if self.opt['distributed']:
                    ...
                    if self.task in ['inpainting','uncropping']:
                        self.output, self.visuals = self.netG.restoration_valid(self.cond_image, y_t=self.cond_image,
                            y_0=self.gt_image, mask=self.mask, sample_num=self.sample_num)
                    else:
                ...
                for key, value in self.get_current_visuals(phase='val').items():
                    self.writer.add_images(key, value)
                self.writer.save_images_valid_test(self.save_current_results())

            for met in self.metrics:
                key = met.__name__

                gt_path = os.path.join(self.writer.result_dir, "val", str(self.epoch), "GT")
                out_path = os.path.join(self.writer.result_dir, "val", str(self.epoch), "Out")

                value = met(gt_path, out_path)
                self.val_metrics.update(key, value)
                self.writer.add_scalar(key, value)

            val_FID_loss = self.val_metrics.result()['val/FID']
            val_FID_loss = torch.tensor(val_FID_loss, dtype=torch.float32)

            val_LPIPS_loss = self.val_metrics.result()['val/LPIPS']
            val_LPIPS_loss = torch.tensor(val_LPIPS_loss, dtype=torch.float32)

            # 保存最好的checkpoint下的图片
            if val_FID_loss.item() < self.opt['train']['min_val_FID_loss']:
                path = os.path.join(self.opt['path']['results'], "best")
                if os.path.exists(path):
                    shutil.rmtree(path)
                self.writer.save_images_best(self.save_current_results())

        # return {'val/FID': val_FID_loss}
        # return {'val/LPIPS': val_LPIPS_loss}
        return {'val/FID': val_FID_loss, 'val/LPIPS': val_LPIPS_loss}
    ...
    def test(self):
        ...
        for met in self.metrics:
            ...
            gt_path = os.path.join(self.writer.result_dir, "test", str(self.epoch), "GT")
            out_path = os.path.join(self.writer.result_dir, "test", str(self.epoch), "Out")
            value = met(gt_path, out_path)

            self.test_metrics.update(key, value.item())
