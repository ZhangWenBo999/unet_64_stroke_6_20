...
from lama_pl.ade20k import resnet, mobilenet
...
class ModelBuilder:
    ...
    def get_decoder(weights_path, arch_encoder, arch_decoder, fc_dim, drop_last_conv, *arts, **kwargs):
        path = os.path.join(weights_path, '', f'ade20k-{arch_encoder}-{arch_decoder}/decoder_epoch_20.pth')
    ...
    def get_encoder(weights_path, arch_encoder, arch_decoder, fc_dim, segmentation,
                    *arts, **kwargs):
        if segmentation:
            path = os.path.join(weights_path, '', f'ade20k-{arch_encoder}-{arch_decoder}/encoder_epoch_20.pth')
        else:
            path = ''
